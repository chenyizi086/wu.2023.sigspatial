...
class UNETSpa(nn.Module):
    ...
    def forward(self, input_temporal, input_spatial, return_att=False):
        sz_b, seq_len, d, h, w = input_spatial.shape
        x = rearrange(input_spatial, 'b t d h w -> (b t) d h w')
        ...
        x5_out = rearrange(x5_out,  '(b s) d h w -> b s d h w', b=sz_b, s=seq_len)

        # Spatial multi-head attention
        spa_attn_out, spa_attn = self.spa_vit(x5_out)

        # Residual connection (add value before normalization)
        out = rearrange(x5, '(b s) c h w -> b s c h w', b=sz_b, s=seq_len)[:, 4]
        out = spa_attn_out + out
        ...
        # Upsample
        x4 = rearrange(x4, '(b s) c h w -> b s c h w', b=sz_b, s=seq_len)[:, 4]
        x = self.up1(out, x4) # Only center patch is used

        x3 = rearrange(x3, '(b s) c h w -> b s c h w', b=sz_b, s=seq_len)[:, 4]
        x = self.up2(x, x3)

        x2 = rearrange(x2, '(b s) c h w -> b s c h w', b=sz_b, s=seq_len)[:, 4]
        x = self.up3(x, x2)

        x1 = rearrange(x1, '(b s) c h w -> b s c h w', b=sz_b, s=seq_len)[:, 4]
        x = self.up4(x, x1)
        ...
        if return_att:
            spa_attn = spa_attn.max(3)
            sz_b, seq_len, n_head, xy = spa_attn.shape
            spa_attn = spa_attn.reshape(sz_b, seq_len, n_head, 4, 4)
            return logits, spa_attn
        else:
            return logits
