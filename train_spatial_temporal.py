...
from model.spatem.spatem import UNETSpaTem
from loss.dice_loss import DiceLoss
import matplotlib.pyplot as plt
from viz.vis import visualize
import random

import log

def train(args):
    model = UNETSpaTem(n_channels=3, n_classes=4, n_head=args.n_head)
    ...
    test_pred_save_path = os.path.join(res_dir, 'test_pred')
    ...
    for epoch in range(0, epochs):
        ...
        with tqdm(total=int(n_train*args.batch_size)-1, desc=f'Epoch {epoch + 1}/{epochs}', unit='img', bar_format='{desc:<5.5}{percentage:3.0f}%|{bar:10}{r_bar}') as pbar:
            for i, (spatial_ID_imgs, temporal_ID_imgs, anno_ID_img) in enumerate(trainloader):
                ...
                out, attn_temp, attn_spatial  = model(temporal_ID_imgs, spatial_ID_imgs, return_att=True)
                ...
                if i % 20 == 0:
                    visualize(temporal_ID_imgs, anno_ID_img, out, attn_temp, attn_spatial, spatial_ID_imgs, sample_save_path, 1, i)
...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='unet-spa-tem',
